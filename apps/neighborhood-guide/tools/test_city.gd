...
func run_checks() -> void:
	...
	check(city.cafe_roof != null, "Detailed cafe must expose a roof group for inspection")
	check(city.get_node("CafeBuilding").find_child("CafeCounter", true, false) != null, "Cafe must contain its service counter")
	city.focus_cafe()
	check(city.get_node("Camera").size < 20, "Cafe focus must bring the camera close")
	city.toggle_cafe_roof()
	check(not city.cafe_roof.visible, "Roof-off control must reveal the interior")
	city.show_district()
	check(city.cafe_roof.visible and city.get_node("Camera").size > 30, "District view must restore the roof and camera")
