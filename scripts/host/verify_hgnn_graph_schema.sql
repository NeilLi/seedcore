...
UNION ALL SELECT 'task_embeddings', CASE WHEN to_regclass('public.task_embeddings') IS NOT NULL THEN 'OK' ELSE 'MISSING' END
ORDER BY view;
...
UNION ALL SELECT 'create_graph_embed_task_v2(integer[],integer,text,text,text)', CASE WHEN to_regprocedure('create_graph_embed_task_v2(integer[],integer,text,text,text)') IS NOT NULL THEN 'OK' ELSE 'MISSING' END
UNION ALL SELECT 'create_graph_rag_task_v2(integer[],integer,integer,text,text,text)', CASE WHEN to_regprocedure('create_graph_rag_task_v2(integer[],integer,integer,text,text,text)') IS NOT NULL THEN 'OK' ELSE 'MISSING' END
ORDER BY function;
...
SELECT * FROM task_embeddings LIMIT 0;
