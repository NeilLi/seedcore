#!/usr/bin/env python3
"""
Comprehensive SeedCore Database Pooling & Feature Test Script

This script provides comprehensive testing of the SeedCore system with focus on:
...
Requires: docker Python library, requests
"""
...
class SeedCoreTester:
    ...
    def test_docker_services(self) -> Dict[str, Any]:
        ...
        for service in expected_services:
            try:
                ...
                self.log(f"  {service}: {status} ({health})")
    ...
    def test_api_endpoints(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.log(f"  Health endpoint: {'✅' if response.status_code == 200 else '❌'}")
        except Exception as e:
            results["health"] = {"success": False, "error": str(e)}
            self.log(f"  Health endpoint: ❌ - {e}", "ERROR")
        ...
        try:
            ...
            self.log(f"  System status: {'✅' if response.status_code == 200 else '❌'}")
        except Exception as e:
            results["system_status"] = {"success": False, "error": str(e)}
            self.log(f"  System status: ❌ - {e}", "ERROR")
        
        # Test organism status
        try:
            response = requests.get(f"{self.base_url}/organism/status", timeout=10)
            results["organism_status"] = {
                "success": response.status_code == 200,
                "status_code": response.status_code,
                "data": response.json() if response.status_code == 200 else None
            }
            self.log(f"  Organism status: {'✅' if response.status_code == 200 else '❌'}")
        except Exception as e:
            results["organism_status"] = {"success": False, "error": str(e)}
            self.log(f"  Organism status: ❌ - {e}", "ERROR")
    ...
    def test_ray_cluster(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.log(f"  Ray status: {'✅' if response.status_code == 200 else '❌'}")
        except Exception as e:
            results["ray_status"] = {"success": False, "error": str(e)}
            self.log(f"  Ray status: ❌ - {e}", "ERROR")
