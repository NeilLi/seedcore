...
# REMOVED: _get_task_queue function - no longer needed since task processing is handled by queue_dispatcher.py
...
# REMOVED: _task_worker function - task processing is now handled by the dedicated queue_dispatcher.py
# The tasks router should only handle API endpoints for task creation/retrieval, not task processing.
...
async def create_task(
    ...
    if run_immediately:
        logger.debug(
            "Task %s marked as QUEUED for processing by queue_dispatcher",
            new_task.id
        )
...
# REMOVED: Coordinator health check endpoint - coordinator interaction is now handled by queue_dispatcher.py