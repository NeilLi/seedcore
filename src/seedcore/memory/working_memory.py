...
def get_miss_tracker():
    ...
    if _miss_tracker is None:
        try:
            ...
            _miss_tracker = MissTracker.options(name="miss_tracker").remote()
...
def get_shared_cache():
    ...
    if _shared_cache is None:
        try:
            ...
            _shared_cache = SharedCache.options(name="shared_cache").remote()
...
class MwManager:
    ...
    def get_item(self, item_id: str) -> str | None:
        ...
        global_value = ray.get(shared_cache.get.remote(global_key))
        if global_value:
            logger.info(f"[{self.organ_id}] Mw HIT for '{item_id}' in GLOBAL cache.")
            return global_value
            
        # Check in-memory organ-local cache
        key = self._get_organ_key(item_id)
        value = self._cache.get(key)
        if value is not None:
            logger.info(f"[{self.organ_id}] Mw HIT for '{item_id}' in ORGAN cache.")
            return value
        # On a miss, log the event in Ray actor (fire-and-forget)
        logger.warning(f"[{self.organ_id}] Mw MISS for '{item_id}'. Logging miss.")
        self.mw_store.incr.remote(item_id)  # Fire-and-forget, no ray.get()
        # Also track in global miss counter
        miss_tracker = get_miss_tracker()
        miss_tracker.incr.remote(item_id)  # Fire-and-forget
        return None
