...
def _validate_dataset(dataset: Any) -> Tuple["pd.DataFrame", str]:
    ...
    raise ValueError(
        "Unsupported dataset type. Provide a pandas DataFrame or dict with 'X' and 'y'."
    )
...
def tune_xgb_trainable(config: Dict[str, Any]) -> None:
    ...
    trial_id = (
        ...
        else f"trial_{int(time.time() * 1000) % 10000}"
    )
    ...
    try:
        ...
        from src.seedcore.ml.tuning_service import (  # pyright: ignore[reportMissingImports]
            _validate_dataset,
        )  # re-use same helper
        ...
        logger.info(
            f"✅ Trial {trial_id} completed - AUC: {auc:.4f}, LogLoss: {logloss:.4f}, Time: {training_time:.2f}s"
        )

        # Report the score back to Ray Tune
        logger.info(
            f"📊 Reporting metrics for trial {trial_id}: mean_accuracy={auc}, logloss={logloss}"
        )
...
class HyperparameterTuningService:
        ...
        try:
            ...
            if dataset is not None:
                try:
                    ...
                    logger.warning(
                        f"⚠️ Provided dataset invalid ({e}); falling back to synthetic."
                    )
                    attached_dataset = None
            ...
            if progress_callback:
                progress_callback(
                    f"Configuring search space with {tune_config_dict['num_samples']} trials..."
                )
            ...
            search_alg = BasicVariantGenerator(
                max_concurrent=tune_config_dict["max_concurrent_trials"]
            )
            ...
            logger.info(
                f"🏆 Best trial achieved AUC: {best_result.metrics.get('mean_accuracy', 0):.4f}"
            )
            ...
            if self.gpu_guard and job_id:
                try:
                    ...
                    logger.warning(
                        f"⚠️ Failed to complete job {job_id} in GPU Guard: {e}"
                    )
            ...
            if self.flashbulb_client:
                try:
                    ...
                    self.flashbulb_client.log_incident(
                        event_data, 0.8
                    )  # High salience for successful tuning
            ...
            if status_actor_handle and job_id:
                try:
                    ray.get(
                        status_actor_handle.set_status.remote(
                            job_id,
                            {
                                "status": "COMPLETED",
                                "result": {
                                    ...
                                    "best_trial": {
                                        ...
                                        "training_time": best_result.metrics.get(
                                            "training_time"
                                        ),
                                    ...
                                    "total_trials": total_trials,
                                    "experiment_path": str(best_result.path)
                                    if hasattr(best_result, "path")
                                    else "~/ray_results",
                                },
                    ...
                    logger.info(
                        f"✅ Direct status update sent to actor for job {job_id}"
                    )
                except Exception as e:
                    logger.warning(
                        f"⚠️ Failed to send direct status update to actor: {e}"
                    )

            # Mark guard completion (approx wall time as coarse GPU seconds)
            gpu_secs = max(1.0, time.time() - sweep_start)
            self.gpu_guard.complete(
                guard_job_id, actual_gpu_seconds=gpu_secs, success=True
            )
            return {
                "status": "success",
                "experiment_name": experiment_name,
            ...
            if status_actor_handle and job_id:
                try:
                    ...
                    logger.info(
                        f"✅ Direct error status update sent to actor for job {job_id}"
                    )
                except Exception as actor_error:
                    logger.warning(
                        f"⚠️ Failed to send direct error status update to actor: {actor_error}"
                    )
    ...
    def _promote_best_model(self, best_result, experiment_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not trial_id:
                ...
                trial_id = best_result.metrics.get(
            ...
            if trial_path:
                ...
                source_model_path = (
                    self.model_storage_path / best_model_name / "model.xgb"
                )

            # Define a stable, "blessed" path for the best model
            blessed_model_name = "utility_risk_model_latest"
            blessed_model_path = (
                self.model_storage_path / blessed_model_name / "model.xgb"
            )
            ...
            for attempt in range(max_retries):
                if source_model_path.exists():
                    ...
                    logger.info(
                        f"   Waiting for model file (attempt {attempt + 1}/{max_retries}): {source_model_path}"
                    )
                        ...
                        if trial_path:
                            ...
                            if trial_dir.exists():
                                ...
                                if model_files:
                                    ...
                                    logger.info(
                                        f"   Found alternative model file: {source_model_path}"
                                    )
                                    break
            else:
                # All retries exhausted
                logger.error(
                    f"❌ Model file not found after {max_retries} retries: {source_model_path}"
                )
                return {
                    "status": "error",
            ...
            if source_model_path.exists():
                ...
                if trial_path:
                    ...
                    if trial_dir.exists():
                        model_files = list(trial_dir.glob("*.xgb"))
                        logger.info(
                            f"   Found model files in trial directory: {model_files}"
                        )
                        ...
                        if model_files:
                            ...
                            shutil.copy2(source_model_path, blessed_model_path)
                            logger.info(
                                f"✅ Model promoted from alternative path: {source_model_path}"
                            )
    ...
    def _convert_dict_to_tune_space(
        self, search_space_dict: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for key, value in search_space_dict.items():
            if isinstance(value, dict) and "type" in value:
                ...
                if value["type"] == "loguniform":
                    converted_space[key] = tune.loguniform(
