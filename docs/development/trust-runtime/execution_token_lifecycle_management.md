...
| Delegated execution | Parent token can be narrowed into a shorter-lived delegated subtoken for Kafka delegated-intent execution. | `agent_action_gateway_contract.md`, `../integrations/kafka_delegated_intent_ingress.md` |
| Attempt vs closure | Token permits an attempt; hardware/signed telemetry and verifier replay decide whether closure settles. | `../evidence/hardware_anchored_telemetry_mvp_contract.md` |
| Replay and proof | Allow-path replay requires execution-token presence and validity in the Rust replay chain. | `../infrastructure/language_evolution_map.md`, `../infrastructure/rust_workspace_proposal.md` |
| Revocation and quarantine | RESULT_VERIFIER fail-closed path attempts token revocation before quarantining authoritative twin state. | `../operations/result_verifier_quarantine_remediation_runbook.md` |
...
4. Link `../evidence/hardware_anchored_telemetry_mvp_contract.md` to RATS vocabulary when
   adding real attestation results.
5. Link `../operations/result_verifier_quarantine_remediation_runbook.md` to token-revocation
