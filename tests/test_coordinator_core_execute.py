...
class TestExecuteTask:
    ...
    async def test_query_routes_via_organism_even_when_cognitive_client_exists(self):
        """Coordinator should not short-circuit query execution directly to CognitiveService."""
        task = TaskPayload(
            task_id="task-query-organism-route",
            type="query",
        ...
        surprise_computer = FakeSurpriseComputer(return_value={
            ...
            "decision": "fast",
            "decision_kind": "fast",
            "ocps": {"S_t": 0.0, "h": 0.5, "flag_on": False},
        })
        ...
        cognitive_client.execute_async = AsyncMock(return_value={
            "success": True,
            "result": {"thought": "should not be used for direct query shortcut"},
        })
        ...
        assert result["decision_kind"] == DecisionKind.FAST_PATH.value
        organism_execute.assert_awaited_once()
        cognitive_client.execute_async.assert_not_awaited()
